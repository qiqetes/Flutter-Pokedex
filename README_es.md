...
- **Freezed** para definir modelos fácilmente
- **PokeApi** para peticiones usando GraphQL

Era un proyecto de fin de semana, así que el código no es perfecto. Sigue una arquitectura MVC (también hay una rama `feat/MVVM` con arquitectura MVVM).

Puede cachear las búsquedas de la API, por lo que la misma búsqueda no disparará una nueva petición, + desplazamiento infinito.
- **Riverpod** como gestor de estado
- **GoRouter** para navegación
- **Freezed** para definir modelos fácilmente
- **PokeApi** para peticiones usando GraphQL

**Cositas interesantes:** Puede cachear las búsquedas de la API, por lo que la misma búsqueda no disparará una nueva petición; Desplazamiento infinito; Diseño responsive;
